...
def test_claim_emits_arc28_claimed_event(algorand) -> None:
    ...
    fund_account(algorand, holder)
    opt_account_into_asset(algorand, holder, asset_id)
    ...
    assert claimed.abi_return == 750_000

    logs = [base64.b64decode(entry) for entry in claimed.confirmation["logs"]]
    ...
    # concatenation: 32 bytes of address followed by 8 bytes of amount. The
    # address is whoever held the NFT at claim time, not a stored beneficiary.
    payload = events[0][4:]
    assert len(payload) == 40
    assert encode_address(payload[:32]) == holder.address
