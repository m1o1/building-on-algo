...
# Set trace_all=True to capture every transaction instead of only failures.
# AlgoKit AVM Debugger can inspect generated TEAL and grouped transactions.
...
def build(output_dir: Path, contract_path: Path) -> Path:
    ...
    build_result = subprocess.run(
        [
            ...
            "--output-source-map",
            # Pinned here rather than left to the compiler default so a
            # reader cannot forget it. PuyaPy 5.x defaults to AVM 11; every
            # contract in this book targets 12.
