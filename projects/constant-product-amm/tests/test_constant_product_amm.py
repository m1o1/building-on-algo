...
def test_swap_rejects_wrong_asset_and_wrong_receiver(algorand) -> None:
    ...
    transfer_asset(algorand, admin, trader, token_a, 1_000 * MICRO_UNITS)
    with pytest.raises(Exception, match="Receiver is not the pool"):
        pool.send.swap(
            amm_client.SwapArgs(
                input_txn=asset_transfer_arg(
...
def test_bootstrap_requires_asset_references_and_fee_pooling(algorand) -> None:
    ...
    # Both pools come from the same admin and the same program, so each
    # create needs its own note to stay a distinct transaction.
    missing_refs_pool, _ = factory.send.create.bare(
        params=distinct_create_params()
    )
    with pytest.raises(Exception, match="unavailable Asset"):
        missing_refs_pool.send.bootstrap(
            ...
            # AlgoKit Utils fills in missing asset references by default, so
            # without this the call would succeed and prove nothing. Turning
            # population off is what puts the reader's own reference list --
            # the thing this test is about -- back in charge.
