...
**All three columns are builds with `scripts/keeptogether.lua` absent**, so the comparison isolates this mechanism; that filter moves the caption row of the same scan and none of these three. The caveat travels with the table — `codebreak.lua:96-98` carries it too, and a copy of the table without it invites someone to check these figures against the shipped book and find the caption row disagreeing.
