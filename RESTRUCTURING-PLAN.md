...
#### Phase 5a progress — Chapter 6, moving value

**Chapter 6 is the chapter where a contract stops being a calculator and starts holding money, and its spine is a single asymmetry.** A contract has an account, and the contract is that account's only signer — nobody holds its key, so anything that account will ever do, the contract's own code must ask for. Sending is therefore a matter of authorization and fees: the contract decides, and it pays. Receiving is not symmetrical at all. A payment arrives whether or not the contract wanted it, and the only thing the contract can do about an incoming transfer is look at it and refuse to act — which is why the chapter's four questions about an incoming transaction (which asset, how much, from whom, to where) are framed as *evidence* rather than as control. Every defect in the chapter's named failure is a version of one side of that asymmetry being mistaken for the other.

**The named failure is a tip jar that cannot be emptied, and it carries four defects in forty-three lines, three of them on the danger list.** The jar takes tips, counts them, and has exactly one way to get money out — and that one way is unconditionally broken, on a contract whose account has no private key, which is the strongest available argument that a withdrawal path is not a place to be approximately right. The compound-failure framing is deliberate and was the hardest part of the chapter to get right: correcting only the fee leaves a `withdraw` that asks the jar to empty itself down to nothing, and correcting only the subtraction is *worse*, because it looks right and still fails for every amount rather than only for large ones. Two of the four defects are one repair; a third correction — the group bound — answers no defect on the list at all, and is present only because the other two `tip` checks are not worth writing without it. The chapter says all three of those things out loud rather than letting the diff imply a tidy one-to-one mapping it does not have.

**Five examples from the plan's inventory did not survive as separate examples, and the merges are documented rather than silent.** N3-06 and N3-15 became one `asa-self-optin`; N3-09 and N3-12 became one `asa-roles`; N4-10 and N4-11 became one `group-bounds`; N3-14 was folded into `optin-gate-eager`'s prose rather than shipped as its own contract; and N3-11 was demoted entirely, to a paragraph and a gotcha. Every one of those decisions came from the same pressure: §2.3 caps a cluster at six examples and a chapter at twenty-six, and the merged pairs were each teaching one idea from two angles that a single complete contract could hold. The demotion is the one worth flagging for later slices — N3-11 earned prose, not thirty-five lines, and the plan's inventory should not be read as a promise that every listed item becomes a numbered example.

**Three review agents ran against the chapter and then all three ran again, and the second round is what makes this slice worth recording.** algorand-expert's blocking findings in the first round were a `33.3A` balance literal that go-algorand actually renders as `33.300000A`, a local-schema MBR attributed to the application account when it is billed to the opting-in account, and — most consequentially — a retraction of my own compound-failure framing on `data.IsZero()` grounds. teaching-pro's B-1 caught that rewriting The Problem had left the cluster-5 closer asserting something the new story contradicted, which is exactly the failure mode of editing a chapter's opening after its body is finished. publishing-pro returned two blocking findings: an elision paragraph whose arithmetic did not add up and which had silently dropped an artifact while being rewritten, and an internal contradiction between the chapter's prose and one of its own gotchas.

**That second blocking finding produced the mechanism this slice should be remembered for.** publishing-pro flagged the contradiction and then correctly declined to resolve it, on the grounds that deciding which side was right was a domain call and not an editorial one. CLAUDE.md gives algorand-expert the win on technical matters without exception, so I fixed the gotcha rather than the prose — and then, instead of shipping that fix, sent it back to algorand-expert for confirmation. The expert ruled the fix wrong: too narrow, and it wrote a second independent failure reason out of the book. `IsZero()` requires an account to be *entirely* empty, not merely zero-Algo, so an application account holding even one asset owes 200,000 microAlgo and a full-balance payment is refused at group conclusion even with `fee=UInt64(0)` — with a different error type, from a different layer, that *does* name the minimum. Since the whole back half of the chapter is about contracts that hold assets, that was the case the chapter most needed. The rule to carry: **when the resolution of an agent conflict is itself a technical claim, it needs the expert's sign-off, not merely the expert's precedent.**

**The gotcha was retitled to "An application account's balance is not what it can spend," and the title was chosen to fit the anchor rather than the other way round.** `#spendable-is-not-the-balance` did not move, so no cross-reference anywhere in the book broke and only the generated appendix needed regenerating. That is worth doing deliberately every time a gotcha is retitled: check 14 catches the appendix drift, but nothing catches a silently-renamed anchor except the reference that stops resolving.

**One finding was deliberately not applied.** teaching-pro's S-4 proposed trimming the schema-exclusion reason on redundancy grounds, and teaching-pro itself noted that doing so would partially reverse algorand-expert's HIGH-2 wording from the previous round. It was declined on jurisdictional grounds. publishing-pro then independently reviewed the same passage pair and reported that the two statements share no phrase longer than "billed to" and that the repetition reads as reinforcement rather than duplication — "keep both" — which confirms the decision on editorial grounds as well as procedural ones.

**The knowledge base gained sixteen verified findings and one retraction of a container fact.** The largest cluster concerns failure transcripts: `LogicError`'s string form does not contain `logic eval error:` or the `Details:` clause (both are stripped by the regex that parses them), PuyaPy's ARC-56 `sourceInfo` carries only a program counter and never TEAL, so `line_no` is always `None` and no `and Source Line n` clause can ever appear in a transcript this toolchain produces, and any quoted `pc` must be inside the program's actual byte length — which is now a checkable claim rather than a plausible-looking number. Ten go-algorand error literals were verified against source rather than recalled. And the stale "algokit_utils is NOT installed in this container" entry was replaced: it is installed, at 4.2.3, inside the algokit CLI's own tool venv, and its source should be read directly rather than guessed at or delegated to a web-enabled agent.

**Chapter 5's forward-reference debt is paid.** Its prose-only pointer to moving-value is now a real `{{ch:moving-value}}` reference and `build.py concat` resolves it. The pricing-math pair remains outstanding and waits for Part II, per the mechanism recorded in the Chapter 5 note.

**State at the end of the Chapter 6 slice:** `24 files, 133 examples, 197 anchors, 295 references — 0 errors, 51 warnings`, with none of the warnings attributable to `06-c-moving-value.md`; `133 passed, 0 failed, 0 skipped` on the example compile set; 37 passing unit tests; 67 gotchas harvested, six of them belonging to this chapter; and concat clean at 24 chapters.

**Still outstanding in 5a:** Chapter 7, and the closing edit to the project chapter that ends Part I.

#### Phase 5a progress — Chapter 7, proving it works

**Chapter 7 replaces `02-c-testing.md` and keeps its slug.** The old chapter was 1,506 lines of testing-as-a-topic; the new one is a need — *how do I find out whether this is actually right, before it holds money* — and the manifest entry carries `slug: testing` unchanged so the sixteen `{{ch:testing}}` references elsewhere in the book still resolve without a sweep. That is the pattern for every concept chapter that supersedes a v1 chapter rather than adding to the book: keep the slug, rewrite the file, and let the manifest absorb the filename change.

**Its spine is that a contract only tells you why it refused if you built it able to, and a test only proves something if some arrangement of the world turns it red.** Those are the same fact from two directions. An `assert` with no message is a program counter and nothing else; a test that asserts what the code does rather than what it is for stays green through the defect it was written to catch. The chapter's four clusters walk the evidence chain: where an assert message actually goes (a TEAL comment and an ARC-56 `sourceInfo` entry, never the bytecode, and the substitution is client-side), where validation belongs given what the router already proved, how to run a call without committing it, and how to write tests against requirements instead of against the implementation.

**The Mini-Build is the vesting contract from the Part's project, deployed and demonstrably working, with three defects that a thorough-looking suite does not catch.** A claim that would move nothing returns zero and confirms; `total * elapsed` overflows before the divide; and one rejection carries no message. The three are deliberately of different kinds — a semantic hole, an arithmetic hole, and a diagnostic hole — because the point of the chapter is that "the tests pass" is a claim about the tests.

**The container could not run LocalNet, and that constraint shaped the chapter for the better.** `algokit localnet start` fails pulling `algorandfoundation/conduit-localnet` with a proxy 403 — an egress-policy denial, reported rather than routed around. The consequence is a rule worth generalizing: **the chapter prints no on-chain transcript it could not produce.** Everything shown was actually run here — PuyaPy compilation, ARC-56 spec reads, the `algorand-python-testing` emulator, and a source-map lookup tool written for the chapter — and the two places where a real node's behaviour is the subject are described rather than dramatized.

**The three-agent review passed, and then the walkthrough found thirteen more defects, four of them factual.** That ordering is the finding. The reviews read the chapter; the walkthrough ran it, and running it is what caught that the `sourceInfo` table had four rows for two assertions with *three* of the four compiler-generated rather than two; that the claim "not every program counter maps to a source line" is empirically false, since `.puya.map` carries one segment per bytecode byte and all 450 in-range pcs resolve; that Exercise 1's premise misdescribed its own artifact (the broken approval program carries `assert` at 28 program counters, three of them without a `sourceInfo` entry, not the numbers the exercise stated); and that four command blocks were unrunnable as printed. **A chapter that passes three specialist reviews can still be a chapter nobody has executed.**

**The failed-lookup passage is the one to keep.** It had been explained as "not every pc has source" — a comfortable, wrong reason. The true one is better teaching: every pc inside the program resolves, so a failed lookup is telling you the number came from somewhere else, and the two cases you actually meet are a clear-state pc looked up in the approval map and a map rebuilt from a commit other than the one deployed — which resolves to a line number that is real and wrong.

**The security audit found one blocking vulnerability the earlier round had missed, in an example whose subject was safety.** `no_reentrancy.py` keeps balances in local state, and a ClearState transaction always succeeds — the clear program cannot refuse — so a depositor who takes the ordinary "opt out of this application" path in their wallet deletes their own balance and strands the funds with no method able to move them. The example is still the right example; the shortcut is now named in its docstring and in the chapter prose, which is the governing rule stated in the plan: *an example may be incomplete, but it may never be exploitable*. The lesson underneath is that "this example demonstrates a security property" is not evidence that the example is secure.

**The audit also found the Mini-Build's second defect materially under-described, and the corrected version is more frightening than the original.** Above roughly 146,000 tokens the product can overflow but only once `elapsed` has grown; below roughly 585,000 it cannot overflow at the cliff. A supply in that band produces a contract that pays the first claims correctly and bricks partway through the term — `claimable()` included, so the beneficiary cannot even ask what they are owed. A defect that passes a testnet run at a plausible supply and detonates in month nine is the shape the chapter now says to be most afraid of.

**Two findings could not be applied where they were found, and the reason is the line budget.** Check 10's "printed lines" is the raw file line count minus one, and both security notes were docstring additions to `core`-tier examples already near the 35-line cap. `no_reentrancy.py`'s is blocking, so its docstring was recompressed to the original eight-line footprint while gaining the ClearState warning; `validate_at_boundary.py`'s is a scoping note, so it was reverted in the file and rehomed to chapter prose beside the example. **A security note that does not fit in the example belongs in the prose, not nowhere** — and the budget arithmetic should be done before writing, not discovered by the validator.

**Knowledge-base additions worth carrying forward.** PuyaPy 5.9 writes both `.arc56.json` and `.puya.map` by default, so `--output-arc56` is no longer a flag to remember. `algokit-utils` enables a full four-field execution trace plus `allow_more_logs`, `allow_unnamed_resources` and `allow_empty_signatures` on *every* simulate it makes, which means an explicit `SimulateTraceConfig` in reader code narrows the default rather than widening it — the chapter now frames those arguments as documentation of what a call depends on. `LogicError.message` is never the bare assert string; it wraps it with contract, app id and transaction id, so reader code must use `in` and not `==`. And `composer.simulate()` raises rather than returning a failure result.

**State at the end of the Chapter 7 slice:** `24 files, 149 examples, 211 anchors, 326 references — 0 errors, 38 warnings`, none of them attributable to `07-c-proving-it-works.md`; `149 passed, 0 failed, 0 skipped` on the example compile set; 37 passing unit tests; 72 gotchas harvested, six belonging to this chapter; and concat, mdbook and PDF all building clean. The warning count fell from 51 to 38 because `02-c-testing.md` left the book. The chapter's pages were rasterized and read per the gate established in Chapter 1: the simulate-trace anchor diagram, the wrapped program-counter transcripts, and the ARC-56 `sourceInfo` listing all render as intended, and every cross-reference resolves in the built HTML.

**Still outstanding in 5a:** only the closing edit to the project chapter that ends Part I — convert first-teaching prose in `03-p-token-vesting.md` to `{{ex:}}` back-references and regenerate its `## What You Need First` block from the Chapter 1–7 Handoff tables. Two debts land in that same edit: `projects/chapter3/token-vesting/tests/` must be brought up to what Chapter 7's Handoff now says the project needs, and `03-p-token-vesting.md:825` references a `setup_initialized_contract` fixture that no chapter defines.

#### Phase 5a progress — the closing edit, and Part I is done

**Not one line of the vesting contract changed, and that was the constraint that made the edit informative.** §8.1 forbids touching a project's contract code, so every defect the edit surfaced had to be resolved in prose or in tests. The chapter is now the assembly the seven concept chapters were written to make possible: `## What You Need First` is generated from the union of their Handoff tables, first-teaching passages are `{{ex:}}` back-references, and `03-p-token-vesting.md` no longer appears in check 8 — the mechanical test for "a Part is done."

**Building the reciprocal block found seven drifts, and all seven were fixed in the Handoff tables rather than in the block.** Method names that no longer exist (`claimable()`, `schedule_of()`, `configure`), a unit claim that was wrong in both directions ("rounds not seconds"), and three predictions that described guarantees the contract does not make ("does not assert group size", "two of them unused", "Every guard"). §8.4's promise is that the two sides *can never disagree*; the only way to keep that true is to treat the reciprocal block as a read of the tables and repair the tables when the read comes back wrong. Patching the block would have satisfied the reader and broken the invariant.

**The `divw`/`divmodw` conflict is settled, and the resolution is the passage worth keeping.** {{ch:numbers-and-time}} says to prefer `divw` because it fails loudly; the project uses `op.mulw` + `op.divmodw` + `assert q_hi == 0`, and the code may not change. The chapter now proves the two are the same test — writing $N = h \cdot 2^{64} + l$ and case-splitting on $d > h$ — and then says the harder thing: on this contract's control flow the assert is provably dead, verified by deleting it and watching all fourteen unit tests still pass. Exercise 3 was rebuilt around what remains to decide when the tests cannot distinguish two versions. **A conflict between a rule and a codebase is a teaching opportunity only if you resolve it with an argument; resolving it by editing one side to match the other teaches nothing.**

**The gotcha count came into the cap by cutting, not by merging.** Eight against a 3–6 ceiling. `#box-prefix-mbr` and `#block-timestamp-is-approximate` were checked against the concept chapters first, found to be largely re-statements, and cut — with the genuinely novel half of each absorbed into prose (that an omitted `key_prefix` silently changes the MBR through the attribute name; that every clock comparison here is `>=` and never `==`). The rule the count enforces is that a gotcha is a thing the reader has not been told, not a thing worth repeating.

**The three-agent review returned twelve blocking items; a targeted re-review of the fixes found three more, one of them introduced by the fixes.** That last one is the finding. A simulate example written to demonstrate negative testing asserted `"No schedule" in err.message` against a method whose guard says `"No vesting schedule"`, and funded its attacker with `AccountManager.random()`, which funds nothing — so the example would have failed inside its own `except` block, in the exact shape the surrounding prose warns against. Two AVM error strings were also plausible paraphrases rather than literals: `box read budget exceeded` has no `box` prefix in go-algorand, and `asset %v missing from %v` is the *sender*-side message where the chapter's scenario is the receiver (`receiver error: must optin, ...`, wrapped as `inner tx %d failed:`). **Every one of the three was a remembered string rather than a looked-up one.** `.claude/agents/algorand-expert.md` gained a ground-truth section and two checklist rules requiring that error literals be grepped out of source with a file and line recorded beside the quote, and that every negative-test example be traced for the failure it actually produces rather than the one it intends.

**The project's test suite is now split along the line the chapter argues for.** `tests/test_vesting_unit.py` is new — fourteen tests, no LocalNet, the clock moved by `patch_global_fields` — and `test_token_vesting.py` holds what needs an AVM. The chapter's prediction answer says where the line runs, and it is not between methods: it runs through the middle of most of them, because a method is usually assertions followed by an effect and only the effect needs a chain.

**State at the end of Phase 5a:** `24 files, 149 examples, 212 anchors, 332 references — 0 errors, 38 warnings`; the two warnings on `03-p-token-vesting.md` are pre-existing check-15 fences that publishing-pro ruled should stay unsplit. `149 passed, 0 failed, 0 skipped` on the example compile set; 37 repository tests plus 14 project tests passing; zero unresolved `{{` templates and zero non-ASCII characters in the chapter; concat, mdbook and PDF building clean. **Part I is complete: seven concept chapters, one project chapter that teaches nothing for the first time, and Handoff tables that agree with the block generated from them.**
