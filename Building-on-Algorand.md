...
The book is structured around six progressively complex chapters, each built incrementally so that every concept is introduced at the moment you need it:
...
- **Chapter 5 --- Common Patterns and Idioms.** A patterns chapter covers cross-cutting production concerns: fee subsidization, MBR lifecycle, canonical ordering, event emission, and opcode budget management.

- **Chapter 6 --- Project 3: A Delegated Limit Order Book with LogicSig Agents.** Algorand's second execution model --- Logic Signatures --- applied to a delegated limit order book. This introduces the hybrid stateful/stateless architecture, template variables, keeper bots, packed binary data, and composability with the AMM from Chapter 4.

- **Chapter 7 --- Project 4: Private Governance Voting with Zero-Knowledge Proofs.** Pushing the AVM to its limits with a private governance voting system using zero-knowledge proofs, elliptic curve operations (BN254), and the MiMC hash. Also covers Algorand's Falcon-based post-quantum security roadmap.
...
Part II applies the foundations to DeFi. You will build a constant product AMM with multi-token accounting, price curves, and LP token mechanics, then study the cross-cutting production patterns --- fee subsidization, MBR lifecycle, event emission --- that separate tutorial code from production code.
...
We used Approach A in the vesting contract's `initialize` method (Chapter 2) and the AMM's `swap` method (Chapter 4). The keeper bot in Chapter 6 uses Approach C (relayer).
...
Both the vesting contract (Chapter 2) and the AMM pool (Chapter 4) use this escrow pattern. The limit order system (Chapter 6) adds a second layer: the LogicSig contract account is also an escrow, but governed by a program instead of an application.
...
This project builds on the LogicSig foundation from Chapter 6. If you skipped that chapter, read at least Part 1 (Logic Signatures) before continuing. Here we recap only the aspects relevant to ZK verification.
...
For this project, we use LogicSigs in **contract account mode** --- the LogicSig program hash determines the account address. The verifier LogicSig does not need delegated authority; it simply needs enough opcode budget to run the elliptic curve operations. The security rules from Chapter 6 (close-to, rekey-to, fee caps, group validation) all apply and are enforced in our verifier implementation.
