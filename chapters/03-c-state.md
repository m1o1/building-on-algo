...
Nothing in that story is a bug in anyone's code. Every line does exactly what it says. The mistake is upstream of the code: it is a belief that "the contract's state" is one store that the contract controls. It is not. It is one slab the application owns outright and one slab per member that the *member* owns, and a member can burn their slab down at any time for any reason, forever, and the application cannot refuse.
...
There is a second, quieter version of the same trap that survives even when the account *is* opted in. `self.credits[member]` compiles down to `app_local_get_ex` followed by an assertion that the key existed. The raw opcode is happy to hand back a zero; PuyaPy is the one that insists the key was really there, on the reasonable theory that you would rather be told. Both readings are covered by the same fix, and {{ex:global-get-default}} is that fix.

Three decisions caused all of this. The four sections that follow take the state model apart, and each one ends by naming what it repairs in the registry. By the end you will be able to state all three in a sentence each.
...
Example: Decoding an untrusted struct argument {#ex:unsafe-decoding}

<!-- finder: validate an ABI struct argument I decoded with validation disabled -->

{{include-ex:unsafe-decoding}}

By default, PuyaPy emits validation code for every ARC-4 argument a method receives: it checks that the caller's bytes really do decode to the declared type before your first line runs. That check costs opcode budget, and for a struct containing a dynamic array it can cost a lot of it. `validate_encoding="unsafe_disabled"` turns it off.

The load-bearing line is `bid.validate()`. Turning off automatic validation does not make the data trustworthy; it moves the validation to a place *you* choose, so you can do it once after a cheap authorization check rather than on every rejected call. Here is the same method with that line missing --- the plausible mistake, and one that a fuzzer finds in minutes:

{{include-ex:unsafe-decoding-wrong}}

With no `validate()`, the length prefix on `rounds` is whatever the caller says it is. The contract will read `bid.amount` out of bytes that were never checked to be a well-formed `Bid`, and the AVM will not stop it, because as far as the AVM is concerned this is just a byte string with an offset applied to it.
...
This is the fix for `credits_of` in the broken registry. A read-only method that a non-member can make fail is a denial-of-service surface on your own dashboard.
...
*What this section repairs in the registry:* `credits_of` reads a key that may not exist and fails when it does not. It should read with a default.
...
There is a second constraint hiding in that innocuous `member: Account` parameter. The AVM can only read the local state of accounts *available* to the transaction. Under consensus v41 an application call may name up to eight accounts in `Txn.Accounts`, plus the sender, plus anything reachable through the unified `Access` list, which holds sixteen entries of any kind. A method that reads local state for a list of members does not scale past that list; a method that reads it for `Txn.sender` always works. This is the first hint that local state is a poor fit for anything you need to iterate over.
...
Read that "inside the global slab" carefully, because it is the map's hard limit. Each entry is one global key/value pair, drawn from the same 64-pair budget the whole application shares. `state_totals=StateTotals(global_uints=16)` means sixteen accounts, ever, and the ceiling is 64 minus whatever else the contract stores. A `GlobalMap` is the right shape for a bounded set --- a handful of admins, a fixed roster of pools, the registry in this chapter --- and the wrong shape the moment the number of keys is a function of how popular you get. For per-account data with no ceiling the answer is boxes, which come with their own chapter; the point here is that moving a record out of local state is not the same as making it unbounded.
...
What that failure looks like depends on how you deploy, and the two paths differ enough to be worth separating. AlgoKit's `deploy()` compares the compiled contract against what is on chain and notices the schema change; `on_schema_break` defaults to `OnSchemaBreak.Fail`, so it raises rather than shipping. A raw `ApplicationUpdate` transaction does not compare anything. It pushes new bytecode to the existing application ID, the network accepts it, and the schema fields on that transaction are ignored, because schema is a creation-time property. Nothing anywhere reports an error. You find out in production, when the tenth uint the new code writes has nowhere to go. The only remedy is a new application, with a new ID, and a migration of every user who opted into the old one.
...
This one does not run on the AVM --- it is a client-side deployment script, and it is here because it is the escape hatch for the mistake the previous example prevents. The load-bearing line is `on_schema_break=OnSchemaBreak.ReplaceApp`.

AlgoKit's `deploy()` compares the compiled contract against what is already on chain and classifies the difference. If only the code changed, `on_update` decides whether to update in place. If the *schema* changed, no update is possible, and `on_schema_break` decides between three outcomes. `Fail` is the default and raises. `ReplaceApp` creates a new application **and deletes the old one in the same group**, which is why it needs the old application to be deletable and why it is catastrophic against a contract holding user funds. `AppendApp` is the one that creates a new application and leaves the old one standing. All three share the same consequence: a new application ID means every opted-in account, every stored balance, and every integration pointing at the old ID is now pointing somewhere else. `ReplaceApp` is a reasonable default on LocalNet and almost never one in production.

*What this section repairs in the registry:* the registry keeps a per-member liability in local state, where the member controls it, and declares a schema with no room to grow. A balance the contract owes belongs in storage the contract owns --- here a `GlobalMap`, which is honest for a roster of this size and would have to become boxes for a roster without a ceiling.
...
The move has a price, and the chapter would be lying to skip it. Sixteen reserved global uints means sixteen creditors, ever, because the global slab tops out at 64 pairs for the whole application. That ceiling is fine for a registry and unacceptable for anything that hopes to grow, and the general answer --- boxes, which are per-key, unbounded, and funded by whoever creates them --- is a chapter of its own. What survives the move to boxes is the reasoning, not the container: the liability must sit in storage the debtor cannot delete.
...
Five, in the order you are likely to meet them: one about counters, one about reads, one about who pays, one about the compiler, and one about the escape hatch in the last example.
...
::: {.gotcha #unsafe-decoding-still-needs-validating topic="Compilation, tooling, and shipping" title="Disabling ARC-4 argument validation moves the validation; it does not remove the need for it"}
...
2. **(Parsons)** Below are five statements. Three of them form the body of a correct `join` method for the fixed registry; two do not belong in it at all. The decorator and `def` line are given, so syntax will not do your ordering for you --- the chapter's rule has to. Select the three that belong, put them in the order that is correct, and for each of the two you rejected, name the specific thing that would go wrong if you kept it.
   ...
   The statements: (a) `self.ever_joined.value += UInt64(1)`; (b) `self.profile[Txn.sender] = Profile(joined_round=arc4.UInt64(Global.round), awards=arc4.UInt64(0))`; (c) `self.credits[Txn.sender] = UInt64(0)`; (d) `assert Txn.sender == self.admin.value, "admin only"`; (e) `assert self.ever_joined.value < UInt64(16), "registry full"`.

3. **(Debug)** A contract declares `state_totals=StateTotals(global_uints=4)` and is deployed. Version two adds a fifth global uint. The developer runs `algokit project deploy localnet`, sees no error, and finds that writes to the new field do not persist. Explain what happened, name the two settings on `deploy()` that determine the outcome, and describe what the developer must do to keep the existing app ID --- or prove that they cannot.
