...
Everything from here on assumes a working toolchain: AlgoKit installed, LocalNet running, and one contract already compiled, deployed, and called. That installation is a twenty-minute errand with no ideas in it, so it lives in {{ch:setup}} rather than in the middle of this chapter. Do it now if you have not already --- every chapter after this one assumes the edit, compile, deploy, interact, test loop is familiar.

One thing that comes out of that setup is worth a section here, because it is a mental-model question rather than a tooling one: what an application call actually looks like on the wire.
...
A contract is reached through one of two clients. The **generic client** --- `get_app_factory` and `get_app_client_by_id`, both used in {{ch:setup}} --- passes method names as strings and leaves arguments untyped:
...
- Read an ARC-56 app spec and explain what an ABI call looks like on the wire
- Choose between the generic and typed AlgoKit clients and say why
