...
Contracts persist data in three places, each with a different owner: **global state** (a fixed schema of up to 64 key/value pairs belonging to the application), **local state** (up to 16 pairs per opted-in account, which that account can delete unilaterally), and **box storage** (independently created keys of up to 32,768 bytes that only the application can remove). Which one you choose is an architectural decision with money attached, and it gets a chapter of its own in {{ch:state}}; the one-line version is that anything the contract *owes* somebody must not live anywhere that somebody can erase.
