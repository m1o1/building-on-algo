...
| Application arguments, all of them together | 2,048 bytes max |
...
Most of the cryptographic opcode costs exceed an application call's 700-unit budget, so they need `ensure_budget` or a group with room in it. Table B-2 lists them, verified against go-algorand v4.7.4-stable.
...
**The unified access list (consensus v41).** Consensus v41 added a single unified access list alongside the separate accounts, assets, and applications arrays (see Table B-5). A transaction uses one form or the other, never both, which is why a modern contract can reference more resources in one transaction than the old per-array caps allowed. It is a wire-format change rather than a source-level one: `algorand-python` has no surface for it at the version this book pins, so it is a fact about what your transactions may carry rather than a line you write.
