...
Raising it is not free, the caller pays, and the amount is not fixed. `ensure_budget` issues op-up inner calls, `OpUpFeeSource.GroupCredit` says they come out of the group's fee credit rather than the application's balance, and there is no credit unless somebody supplies it. Measured: this method needs a `static_fee` of **3,000**, three minimum fees covering the call and each op-up, and at 1,000 or 2,000 it fails with `group fee 0.0A too small (need 1mA)` at the `itxn_submit`. The contract gives no hint of that client-side requirement, and it is the first thing that goes wrong.
