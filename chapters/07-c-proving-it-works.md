...
| {{ex:validate-at-boundary}} | `deposit_tokens`, which takes a typed transfer parameter | The project asserts the transfer's asset, amount and receiver, and does not assert its position or the group size. Say which of those four the router already guaranteed. |
