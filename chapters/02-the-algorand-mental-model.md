...
The minimum [fee](https://dev.algorand.co/concepts/transactions/fees/) is **1,000 microAlgos** per ordinary Ed25519 transaction *today* --- a consensus parameter, not a constant to bake into a client. A Falcon (post-quantum) signature costs three of those floors for the same transaction; oversized notes, arguments, and programs add a per-byte surcharge on top. Fees are validated across the group rather than per transaction: the sum of all fees must cover the group's required total, which means one transaction can overpay to cover others. That is *fee pooling*, and you will use it constantly, because a contract that sends money pays for that send out of the group's fee budget. Chapter 7 teaches the contract-side habit --- never put a non-zero fee on an inner transaction --- and Chapter 8 shows how a client asks `simulate` for the number it should actually attach.
