...
::: {.note}
**Technology stack for this chapter.** This project spans two languages and three components:
...
{{tbl:zk-tracks}} summarizes the two implementation tracks in this chapter.

Table: Chapter implementation tracks {#tbl:zk-tracks}
...
This project builds on the LogicSig foundation from {{ch:limit-order-book}}. If you skipped that chapter, read at least Part 1 (Logic Signatures) before continuing. Here we recap only the aspects relevant to ZK verification.
...
{{tbl:bn254-opcodes}} lists the available opcodes and what each one costs.

Table: Native elliptic curve opcodes and their costs {#tbl:bn254-opcodes}
...
Forgetting a box reference *usually* produces `invalid Box reference 0x...`, with the box's name in hex --- the box is not in the call's available set at all. The two failures are worth keeping apart, and the word doing the work in that sentence is *usually*. A reference buys two things at once: the right to name a box, and 2,048 bytes of I/O allowance. The allowance counts every entry on the group, duplicates and empty ones included, so a call may be carrying references that name nothing and exist purely for budget --- which is what the padding in {{ch:boxes}} is. Drop the reference that *named* a box and you get `invalid Box reference`; drop a padding reference and every name still resolves, so the call gets further and dies on size instead: `read budget exceeded` before the first opcode runs, or `write budget exceeded` partway through. Each names the bytes it wanted and the bytes it had; the write message also names the box in hex and the operation that pushed the count over --- `writing`, `creating` and `resizing` are the only three. Which message you get tells you which kind of reference went missing.

One thing not to rely on: AlgoKit Utils populates app-call resources by default --- `populate_app_call_resources` is `True` unless you turn it off --- so the typed client generated by `algokit generate client` will usually assemble both the references and the padding for you. That default is a convenience, not a guarantee, and it is gone the moment the call is built by something else: another contract, a hand-rolled transaction, a different SDK. Write the references out explicitly when the group has to work without it, which is what the preceding list is for.
...
{{tbl:voting-box-budget}} lists the four kinds of boxes the voting contract uses, each with the fixed key and data layout that fixes its MBR.

Table: Box layout and minimum balance for the voting contract {#tbl:voting-box-budget}
...
{{tbl:zk-summary}} lists what each component does and the concepts it introduced.

Table: Components built and concepts introduced {#tbl:zk-summary}
...
{{tbl:crypto-opcode-costs}} collects the per-operation costs.

Table: Opcode costs for cryptographic operations {#tbl:crypto-opcode-costs}
...
{{tbl:contract-vs-logicsig}} sets the two execution modes side by side.

Table: Smart contracts and LogicSigs compared {#tbl:contract-vs-logicsig}
...
{{tbl:zk-further-reading}} collects the primary sources for the cryptography in this chapter.
